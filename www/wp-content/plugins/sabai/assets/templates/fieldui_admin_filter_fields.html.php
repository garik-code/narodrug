...
<?php   foreach ($filterable_fields as $field):?>
                <a href="<?php echo $form_create_field_url;?>" data-field-id="<?php Sabai::_h($field['field']->getFieldId());?>" class="sabai-btn sabai-btn-default" data-modal-title="<?php echo Sabai::h($field['field']->getFieldLabel())?>" title="<?php echo Sabai::h($field['field']->getFieldLabel() . ' (' . $field['field_type'] . ')')?>"><?php echo Sabai::h($field['field']->getFieldLabel())?></a>
<?php   endforeach;?>
            </div>
        </div>
<?php endif;?>
