               ...
               Sabai_Addon_Form_IFields
{
    ...
    public function managePostsCustomColumn($column, $post_id)
    {
        switch ($column) {
            case 'sabai_voting_updown':
                if (($entity = $this->_application->Entity_Entity('content', $post_id))
    ...
    {
	    parent::upgrade($currentVersion, $log);

        if (version_compare($currentVersion, '1.4.0', '<')
            && version_compare($this->getVersion(), '1.4.0', '>=')
        ) {
            $db = $this->_application->getDB();
            $db->exec(sprintf('UPDATE %svoting_vote SET vote_ip = MD5(vote_ip);', $db->getResourcePrefix()));
        }
    }
}
