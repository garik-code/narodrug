               ...
               Sabai_Addon_System_IPermissions
{
    ...
    public function fieldGetType($name)
    {
        switch ($name) {
            ...
            case 'content_post_user_id':
                return new Sabai_Addon_Content_AuthorFieldType($this, $name);
            default:
    ...
    private function _isPostRoutable($bundle, $entity)
    {
        if (!isset($bundle->info['public']) || $bundle->info['public'] !== false) {
            if ($entity->isPublished() || $entity->isPending()) return true;
        } else {
            if (!$entity->isPublished()) return false;
        }
    ...
    public function uninstall(ArrayObject $log)
    {
        ...
        // Clear view count
        $this->_application->getPlatform()->deleteUserMeta($this->_application->getUser()->id, 'content_viewed');
    }
