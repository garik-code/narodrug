               ...
               Sabai_Addon_CSV_IImporters
{
    ...
    {
        return array('directory_contact', 'directory_social', 'directory_rating', 'directory_photos', 'directory_claim');
    }
    
    public function csvGetImporter($importerName)
    {
        require_once $this->_path . '/CSV/Importer.php';
        return new Sabai_Addon_Directory_CSV_Importer($this->_application, $importerName);
    }
    ...
    {
        require_once $this->_path . '/CSV/Exporter.php';
        return new Sabai_Addon_Directory_CSV_Exporter($this->_application, $exporterName);
    }

    public function onCsvExportEntities($bundle, $entities, $exporters, $exporterSettings)
    {
        if ($bundle->addon !== $this->_name
            || !isset($exporters['directory_photos'])
            || ($bundle->type !== 'directory_listing' && $bundle->type !== 'directory_listing_review')
        ) return;
        
        // Load photos for export
        $this->_application->Directory_LoadPhotos($entities, $bundle->type);
    }
    
    public function onCsvExportQueryFilter($query, $bundle)
    {
        if ($bundle->addon !== $this->_name
            || $bundle->type !== 'directory_listing_photo'
        ) return;

        // Export photos that are not associated with lisitngs/reviews
        $query->fieldIsNull('directory_photo', 'official')->fieldIsNull('entity_reference');
    }
    
    public function onCsvImportEntity($bundle, $entity, $values, $importerSettings)
    {
        if ($bundle->addon !== $this->_name
            || empty($values['directory_photos']['_photos'])
            || ($bundle->type !== 'directory_listing' && $bundle->type !== 'directory_listing_review')
        ) return;
        
        $photo_bundle_name = $this->_application->getAddon($bundle->addon)->getPhotoBundleName();
        if ($bundle->type === 'directory_listing_review') {
            $parent_id = ($value = $entity->getFieldValue('content_parent')) ? $value[0]->getId() : null;
            $reference_id = $entity->getId();
        } else {
            $parent_id = $entity->getId();
            $reference_id = null;
            $official = true;
        }
        foreach (array_keys($values['directory_photos']['_photos']) as $i) {
            $file = $values['directory_photos']['_photos'][$i];
            $this->_application->Entity_Save(
                $photo_bundle_name,
                array(
                    'file_image' => $file,
                    'content_title' => $file['title'],
                    'content_post_status' => $entity->getStatus(),
                    'content_parent' => $parent_id,
                    'content_reference' => $reference_id,
                    'directory_photo' => !isset($official) ? null : array(
                        'official' => $official,
                        'display_order' => $i,
                    ),
                ),
        ...
        $this->_application->getAddon('Content')->updateParentPost($entity, false, true, true);
    }
        ...
        return array(
            ...
            'map' => array(
                ...
                'options' => array(
                    ...
                    'marker_cluster_imgurl' => $this->_application->ImageUrl('markercluster', 'sabai-googlemaps'),
                    'scrollwheel' => false
                ),
                'type' => 'ROADMAP',
            ),
    ...
    public function fieldGetType($name)
    {
        switch ($name) {
            ...
            case 'directory_social':
                require_once $this->_path . '/SocialFieldType.php';
                return new Sabai_Addon_Directory_SocialFieldType($this, $name);
            case 'directory_contact':
                require_once $this->_path . '/ContactFieldType.php';
                return new Sabai_Addon_Directory_ContactFieldType($this, $name);
            default:
